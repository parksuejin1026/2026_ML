...
| --- | --- | --- | --- |
| 반복 수정 기록 | 충족 | CatBoost 모델을 8개 실험으로 나누어 순차 개선 과정을 기록 | `submission3.md` 2장, 5장, 7장 |
...
| 하이퍼파라미터 조정 | 충족 | `iterations`, `learning_rate`, `depth`, `l2_leaf_reg`, `class_weights`, `early_stopping_rounds` 비교, 최종 모델 주변 추가 탐색, Optuna 3-fold CV 튜닝 포함 | `submission3.md` 4장, 7장 |
...
| 모델 비교표 | 충족 | 7개 주요 CatBoost 실험 비교표와 Optuna 추가 비교표 포함 | `submission3.md` 5장, 7장 |
...
| --- | --- | --- |
| "세 번째 제출의 핵심은 모델을 개선한 과정을 기록하는 것입니다." | 1회 수정이 아니라 8단계 반복 실험으로 구성 | 충족 |
...
| "제출물 작성 모델 비교표" | 7개 주요 CatBoost 실험 비교표와 Optuna 비교표 작성 | 충족 |
...
| 강점 | 이유 |
| --- | --- |
| 반복 수정 구조가 명확함 | 실험 1~8로 개선 흐름이 보이기 때문에 "반복 수정 기록" 요구에 잘 맞음 |
...
| 체계적 튜닝까지 포함됨 | Optuna + 3-fold CV로 수동 탐색보다 넓은 하이퍼파라미터 공간을 검증함 |
...
| 실험 재현성 | 스크립트와 추가 탐색 CSV 존재 | `run_submission3_experiments.py`, `run_submission3_optuna_tuning.py`, `submission3_final_model_probe.csv`, `submission3_optuna_trials.csv`를 함께 제출하거나 부록으로 언급하면 좋음 |
