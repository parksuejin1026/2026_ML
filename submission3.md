...
추가로 실험 3~5에는 `early_stopping_rounds`를 적용해 validation 성능이 더 이상 좋아지지 않으면 학습을 멈추도록 했다.
...
최종 모델은 validation set에서 threshold를 탐색한 결과, class 0 확률 기준 **0.46**을 선택했다. 기본값 0.5보다 낮기 때문에 해지 후보를 더 적극적으로 잡는다.
...
| feature selection | 중요도 낮은 파생 변수 제거 후 재학습 | ROC-AUC와 Accuracy 회복 가능 |
