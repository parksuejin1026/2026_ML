...
| 8 | Optuna + 3-fold CV 튜닝 | Bayesian 탐색으로 더 넓은 하이퍼파라미터 공간 검증 |
...
| 8 | Optuna 24 trials, 3-fold CV, `iterations`, `learning_rate`, `depth`, `l2_leaf_reg`, `class_weights`, `threshold` 동시 탐색 | 수동 탐색보다 넓고 체계적으로 최적 조합 탐색 |
...
실험 7의 구체적인 수정 방법은 다음과 같다.

| 수정 항목 | 적용값 | 수정 의도 |
| --- | ---: | --- |
| `class_weights` | `[1.2, 1.0]` | 해지 후보 class 0을 유지 후보보다 1.2배 더 중요하게 학습 |
| `depth` | 4 | 트리 깊이를 낮춰 복잡한 규칙에 과적합되는 문제 완화 |
| `l2_leaf_reg` | 15 | L2 regularization을 강화해 leaf 값이 과도하게 커지는 것을 방지 |
| `learning_rate` | 0.025 | 한 번에 크게 갱신하지 않고 안정적으로 학습 |
| `iterations` | 700 | 낮은 learning rate를 보완할 만큼 충분히 반복 학습 |
| `threshold` | 0.35 | class 0 확률 기준을 낮춰 해지 후보를 더 적극적으로 탐지 |

즉, 실험 7은 **해지 후보를 조금 더 중요하게 학습시키되, class 0 가중치를 과하게 키우지는 않고 정규화를 강화한 모델**이다. 이후 threshold를 0.35로 낮춰 실제 추천 단계에서 해지 후보를 더 민감하게 잡도록 했다.
...
최종 모델을 선택한 뒤에도 성능이 더 좋아질 가능성이 있는지 확인하기 위해, 현재 최고 조합 주변의 일부 하이퍼파라미터를 추가로 비교했다. 탐색 대상은 class weight, tree depth, L2 regularization, learning rate, random strength였다.
...
| depth 3 | 트리 깊이 3으로 축소 | 0.6955 | 0.7743 | 0.5463 | 0.7566 | 0.6345 |
| learning rate 0.03 | 학습률 상승 | 0.6928 | 0.7748 | 0.5429 | 0.7609 | 0.6337 |
| L2 20 | 정규화 강화 | 0.6853 | **0.7752** | 0.5339 | **0.7774** | 0.6331 |

추가 탐색 결과, Recall(0)만 보면 L2 regularization을 더 강하게 준 모델이 0.7774로 높았다. 하지만 Precision(0)과 Accuracy가 크게 낮아져 F1(0)은 최종 모델보다 낮았다. 따라서 해지 후보를 무조건 더 많이 잡는 방향보다는, **Recall(0)과 Precision(0)의 균형이 가장 좋은 현재 모델을 유지**하는 것이 적절하다고 판단했다.

이 추가 탐색은 최종 모델을 바꾸지는 않았지만, 선택한 모델이 단순히 우연히 고른 조합이 아니라 주변 후보와 비교해도 가장 균형이 좋다는 근거가 된다.

### 7-5. Optuna 기반 K-Fold 튜닝

마지막으로 수동 탐색보다 더 체계적인 튜닝을 위해 **Optuna Bayesian Optimization + 3-fold Stratified CV**를 수행했다. 각 trial에서는 CatBoost 하이퍼파라미터를 샘플링한 뒤, fold별 validation set에서 threshold를 선택하고 F1(0)을 평균 내어 최적화했다.
...
| --- | --- |
| `iterations` | 450~1000 |
| `learning_rate` | 0.015~0.055 |
| `depth` | 3~6 |
| `l2_leaf_reg` | 6~30 |
| `random_strength` | 0.5~2.5 |
| `bagging_temperature` | 0~1.5 |
| `class0_weight` | 1.0~1.8 |
| `threshold` | validation set에서 자동 선택 |

Optuna가 찾은 최상위 trial은 다음과 같다.
...
Optuna 모델은 Recall(0)이 0.7795로 가장 높았다. 즉, 실제 해지 후보를 가장 많이 찾아내는 방향으로는 Optuna 모델이 강점이 있었다. 그러나 Precision(0), Accuracy, F1(0)은 현재 최종 모델보다 낮았다. 이번 프로젝트에서는 Recall을 가장 중요하게 보지만, 오탐이 지나치게 늘어나면 추천 품질이 떨어지므로 F1(0)을 함께 고려했다.

따라서 최종 제출 모델은 Optuna 모델로 교체하지 않고, **실험 7 모델을 유지**했다. 다만 Optuna 실험을 통해 현재 모델보다 Recall을 더 높이는 방향이 가능하다는 점과, 그 경우 Precision/F1 trade-off가 발생한다는 점을 확인했다.
...
3차 제출에서는 CatBoost 모델을 총 8단계로 반복 수정했다. 단순히 SMOTE와 threshold 조정만 적용한 모델보다, **CatBoost native categorical 처리와 class weight를 사용한 모델**이 가장 좋은 균형을 보였다. 마지막에는 Optuna + 3-fold CV까지 적용해 더 넓은 하이퍼파라미터 공간도 확인했다.
